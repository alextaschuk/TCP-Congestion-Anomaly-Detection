...
ssize_t rcv_dgram(int udp_fd)
{
    ...
    uint8_t *buf = NULL;
    buf = malloc(1460); // This is bounded in send_segment() by seg_buf
    if (!buf)
    {
    ...
    { // continuously listen for incoming packets
        ...
        switch (target_tcb->fsm_state)
        {
            case LISTEN: // Received a SYN; sends SYN-ACK
                ...
                {
                    LOG_ERROR("[rcv_dgram] Expected SYN flag, none found");
                    break;
                }
            ...
            case SYN_RECEIVED: // Received ACK for SYN-ACK; start sending data
                ...
                if (listen_tcb == NULL)
                {
                    LOG_ERROR("[rcv_dgram] listen_tcb is NULL, cannot transition to ESTABLISHED");
                    break;
                }
    ...
    free(buf); // only reachable by fatal socket error (recvfrom <= 0)
    return rcvsize;
}
